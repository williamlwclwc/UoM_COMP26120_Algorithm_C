...
{
  // your code goes here
  if(source > mygraph->MaxSize || target > mygraph->MaxSize)
  {
    printf("Please check 'source' and 'target'\n");
    return -1;
  }
  // source -> target
  ...
  check(new1, "new_link");
  new1->index = target;
  new1->next = mygraph->table[source].outlist;
  mygraph->table[source].outlist = new1;
  mygraph->table[source].outdegree++;
  // undirected graph: target -> source
  List* new2 = (List*)malloc(sizeof(List));
  check(new2, "new_link");
  new2->index = source;
  new2->next = mygraph->table[target].outlist;
  mygraph->table[target].outlist = new2;
  mygraph->table[target].outdegree++;
  return 0;
}
